...
| **2.1 — LRs, prior and threshold hardcoded** | Chapter 7, **closed Chapter 14** | Chapter 7 built the artifacts §6 requires — a corpus, immutable splits, and computable `corpus_hash` / `split_hash` filling the `calibration_runs` columns that had held nothing since Chapter 3 (D-045). Chapter 14 fitted the numbers into them. Every likelihood ratio comes from the calibration split with Laplace smoothing and recorded per-cell counts; the alert threshold from a base-rate-weighted precision–recall sweep on the validation split; both live in `calibration.json` beside the corpus hash they were measured against. The hand-set constants are **deleted, not relabelled** — there is no `PROVISIONAL_RATIOS`, no `PROVISIONAL_PRIOR`, no `PROVISIONAL_ALERT_THRESHOLD` and no `FALLBACK_RATIOS`, so a missing artifact raises rather than silently producing a number nobody measured, and `test_no_unfitted_numbers.py` parses the whole tree to keep it that way (D-080, D-082, D-084). The prior remains **declared** rather than measured, and says so on the artifact's face: a twin-paired corpus has a prevalence of 0.5 by construction, and the ratios it does supply are prevalence-invariant (D-083) |
...
| 4.4 — missing agents degrade silently to "clean" | Chapter 9, **completed Chapter 13** | `apps/worker/analysis.py` fills every one of four slots, substituting an abstaining stand-in for any agent that will not import, and guarantees at least one statement per agent per unit. The comment says "no finding" only when agents actually ran and found nothing, and never on the strength of agents that could not run. No longer partial: as of Chapter 13 all four agents exist and are measured, so the stand-in path is reached only by a loader that genuinely fails — a broken install, a missing extra, a renamed module — rather than by a seat nobody had built yet. `runtime.sfi` on a machine with no WASI interpreter is the live example, and it abstains under its own name rather than degrading to silence (D-077) |
| 3.12 — a missing API key reports "clean" | Chapter 9, **completed Chapter 11** | `SemanticAgent` no longer substitutes `StubLLMClient` when no key is configured: it abstains with `llm_unavailable` on every unit. Found by running the pipeline end to end, and fixed here rather than in Chapter 11 because Chapter 9 is what made it harmful — once fusion consumed silence, a witness that had read nothing was arguing at LR 0.50 that every change was safe (D-057). Chapter 11 closed the neighbouring case: three provider failures now abstain with `provider_unavailable` rather than `schema_violation`, so a transport outage is not recorded as evidence about the model's output when Chapter 14 fits the ratios (D-065) |
| **3.1 — the def-use graph is built and discarded** | **Chapter 10** | `taint/defuse.py` builds a graph from the AST whose nodes are definitions, guards and sink arguments, and `taint/engine.py` propagates over it with a worklist and asks `networkx` for the path it renders. The old builder's result was assigned to a local and never referenced, `import networkx` was unused, and symbol extraction sat behind a literal `if False` — `symbols.py` is deleted (D-059) |
| **3.2 — "taint paths" are a line-number cross-product** | **Chapter 10** | Reachability is a real path from an origin to a sink argument. A source and an unrelated sink below it produce nothing; a three-hop flow renders three steps with a genuine `propagation` role, which the old always-two-step artifact could never contain |
| **3.3 — detection is raw regex over untokenised lines** | **Chapter 10** | Sink patterns `fullmatch` the **callee** of a call node and its dotted suffixes; keyword predicates read the call's actual keyword arguments. `# TODO: replace os.system with subprocess` matches nothing, `eval` no longer matches `evaluate`, and a `shell=True` or `Loader=SafeLoader` on a following line is seen (D-059) |
| **3.4 — sanitizers ignore their class** | **Chapter 10** | Every sink declares a `class` and `RuleSanitizer.clears` is read against it: an edge that clears `xss` is removed from the graph used for an XSS sink and kept for a command sink, so `html.escape` no longer silences an `os.system`. Clearing is computed per occurrence, so `render(escape(a), b)` clears for `a` and not `b`. Rule models are `extra="forbid"`, so a mistyped field fails loudly (D-059) |
| **3.5 — parameterised SQL is modelled as a sanitizer** | **Chapter 10** | `args: [0]` on `sql.execute` with `safe_when: params_passed_separately` naming the choice, as §5 requires. It is the only model that gets `cwe-089-order-sort` right, where both twins pass parameters separately and the vulnerable one concatenates a tainted column into argument 0 (D-059) |
| **3.6 — no type-coercion sanitizers** | **Chapter 10** | `int`, `float`, `uuid.UUID`, `Decimal`, `datetime.fromisoformat` and allowlist-mapping lookups clear every class. §5 calls these "the largest false-positive source without them" and there were none |
| **0.3 — the prompt-injection delimiter can be closed by untrusted input** | **Chapter 11** | The untrusted region is bounded by `CODESHERIFF-` plus eight bytes from `secrets`, drawn per request, and the prompt names any in-block claim to close it as the attack. The exemplars are rendered inside the same sentinel, so nothing teaches the model that the delimiter is decorative. Measured: **0% injection subversion** across the ten calibration cases that carry a baseline detection, against a ≤ 10% criterion (D-066, D-068) |
| **0.4 — no rationale screening** | **Chapter 11** | `screening.py`, applied in `mapping.py` — the only place an `LLMFinding` becomes an `Evidence`, so no path to a row, a comment or the dashboard skips it. Prose that reads as an instruction is dropped whole rather than escaped, and the finding is reported from its gate-validated fields instead; escaping would still publish the payload to the reader it is aimed at (D-067) |
| **3.9 — the anti-sycophancy exemplars are never loaded** | **Chapter 11** | `exemplars.py` loads them, validates each against the schema the model must satisfy, and renders them into every prompt. They could not have been loaded as they stood — each file held only a `response`, with no record of the code it answered — so the input half was authored, and none of it is drawn from the corpus (D-047). Four of six report nothing, and `exemplar_balance` asserts the ratio so it cannot drift |
| **3.10 — the hallucination gate has 3 of 4 checks, two weakened** | Chapter 2, **completed Chapter 11** | All four hold in `mapping.py`: the `IN_SCOPE_CWES` check that was absent, an exact file-path comparison rather than a basename fallback, the verbatim sink check, and evidence-line bounds with the `+ 5` slack removed. A rejected finding is dropped without failing the sample, since a response may carry one real finding and one invention |
| **3.11 — no size check; oversized units return silence, not abstention** | **Chapter 11** | `MAX_UNIT_BYTES` at 60 kB, checked before any call, returning an `unit_too_large` abstention. Never truncation: a truncated unit yields confident findings from half-read code, biased toward over-reporting, and silently invalidates calibration (D-015) |
| **0.2 — no repository isolation in the vector store** | **Chapter 12** | The global Chroma collection is deleted along with the rest of `rag/`. Retrieval is a Protocol the agent declares and `apps/worker` implements against pgvector, constructed against one `repository_id` and holding no parameter that could name another; `search_precedents` filters in SQL. `unit.repo` is deliberately not consulted — the binding comes from the audit's own repository row rather than from a string on an object the agent was handed (D-072) |
| **3.7 — the context agent's reasoning is four substring tests** | **Chapter 12** | Replaced wholesale. `controls.py` reads a control surface off the syntax tree — decorators and called names, so a decorator in a comment, a docstring or a nested helper is not one — `regression.py` mines which controls this repository's own merged history establishes, and `classify.py` decides which of those are authorization. Retrieved excerpts now decide what is detected; with an empty history the agent produces nothing at all. `reasoning/prompts/cross_pr_v1.md`, referenced by no code, is deleted rather than wired up. Measured on the calibration split: **5/5 recall** on the cases `detectable_by` predicts, **0 false positives** across 11 twins and negative controls (D-071) |
| **3.8 — embeddings silently degrade to MD5 hashing** | **Chapter 12** | There is no fallback branch left to take. The agent embeds nothing; the model lives in `apps/worker` beside the store, `sentence-transformers` is declared by the package that uses it, and a missing library, an unloadable model or a wrong output dimension each raise `RetrievalUnavailableError`. That reaches the record as a `retrieval_unavailable` abstention, distinct from `no_precedent` — a store that is down is not a repository that is new, and conflating them is what let a term-hasher report forever that every repository it saw had no history (D-072) |

**Tier 2 is closed.** 2.1 was the last of it, and Chapter 14 fitted the numbers the arithmetic had
been waiting for. `apps/worker/comment.py` still has no code path that renders a posterior without
its calibration state — the banner now names the corpus the ratios were fitted on, the split the
threshold was selected on and the base rate the posterior is stated at, and falls back to
"uncalibrated run" when an audit's calibration row is not a fitted one (D-032, D-080).

Two caveats travel with the fitted numbers, both recorded in the artifact rather than in a footnote.
`structural.semgrep` has no Windows build and abstained on every unit, so the structural ratios
describe a **one-backend witness**; Chapter 18 must re-observe on Linux or CI before a structural
number reaches the paper. And `structural.detection_medium` fits above `detection_high` because 13
of the taint engine's 14 true detections score in the medium band and exactly one scores high — a
scoring-calibration question for the static agent, visible only because every fitted ratio carries
its counts.
