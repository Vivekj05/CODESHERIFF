...
constant is used because this is a direct observation and not an estimate.
If the value reached the sink only because the sandbox had to answer a
guard it could not evaluate, the agent abstains.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{codesheriff_workflow.png}
...
ratios into a posterior probability and computes the Disagreement Index to
produce the final confidence-aware vulnerability verdict.

When a vulnerability is identified, the patching module generates a
candidate fix, which is then re-examined by the deterministic agents prior
to being suggested. The resulting audit outcome, comprising the verdict,
confidence score, supporting evidence, and, where applicable, patch
information, is returned to both the GitHub pull request and the dashboard.
As depicted by the feedback loop in Fig.~\ref{fig:workflow}, results
recorded on the dashboard are subsequently fed back into the parallel agent
analysis stage, enabling the repository's own history to inform the
evaluation of future pull requests. This workflow integrates security
analysis directly into the existing development process, providing
developers with actionable feedback without disrupting standard practice.
