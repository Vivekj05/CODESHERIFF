...
A copy of `apps/api/tests/payloads.py`. The two test suites cannot import each other for the same
reason the two packages cannot — they are separate distributions on the same `import-linter` layer,
and a test tree is not an importable package of either. The module is a generic, schema-driven
generator with no knowledge of either app, so the copy has nothing app-specific to drift on.
